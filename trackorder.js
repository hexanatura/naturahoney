...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function processOrderData(order) {
    ...
    if (order.createdAt) {
      timelineDates.placed = formatFirebaseDate(order.createdAt, true);
    }
    ...
    if (order.confirmedAt) {
      ...
      timelineDates.confirmed = timelineDates.placed || formatFirebaseDate(new Date(), true);
    }
    ...
    if (order.shippedAt) {
      ...
      timelineDates.shipped = timelineDates.confirmed || timelineDates.placed || formatFirebaseDate(new Date(), true);
    }
    ...
    if (order.outForDeliveryAt) {
      ...
      timelineDates['out-for-delivery'] = timelineDates.shipped || timelineDates.confirmed || timelineDates.placed || formatFirebaseDate(new Date(), true);
    }
    
    if (order.deliveredAt) {
      timelineDates.delelled = formatFirebaseDate(order.deliveredAt, true);
    } else if (order.status === 'delivered') {
      timelineDates.delivered = timelineDates['out-for-delivery'] || timelineDates.shipped || timelineDates.confirmed || timelineDates.placed || formatFirebaseDate(new Date(), true);
    }
